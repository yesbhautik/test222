...
const generate = async ( message_no, creativity, maximum_length, number_of_results, prompt ) => {
	...
	if (stream_type == 'backend') {
		const response = await fetch( streamUrl + 'Post', {
			method: 'POST',
			headers: {
		...
		while (streaming)
		{
			...
			for ( const parsedLine of parsedLines ) {
				if ( parsedLine != null && parsedLine.message != null) {
					chunk.push( parsedLine.message.replace(/(?:\r\n|\r|\n)/g, ' <br> ') );
				}
			}
		}
	} else {
...
const tinymceOptions = {
	...
	statusbar: false,
	plugins: [
		'advlist', 'link', 'autolink', 'lists', 'code', 'wordcount', 'image'
	],
	...
	toolbar: 'styles | magicIconRewrite | magicAIButton |  link | image | forecolor backcolor emoticons | bold italic underline | bullist numlist | blockquote | wordcount | alignleft aligncenter alignright',
		...
		const menuItems = {
			...
			'rewrite': {
				...
				onAction: function () {
					...
					Alpine.store('appLoadingIndicator').show();
					let formData = new FormData();
					formData.append( 'prompt', 'Rewrite below content professionally. Keep origin language.' );
					formData.append( 'content', editor.selection.getContent() );
					$.ajax( {
						type: 'post',
						url: '/dashboard/user/openai/update-writing',
