...
class DocumentsApiController extends Controller
{
    ...
    public function getDocs(Request $request)
    {
        ...
        if ($request->sort != null) {
            if (in_array($request->sort, ['newest', 'oldest', 'az', 'za']) == false) {
                return response()->json(['error' => 'Invalid sort parameter. Must be one of newest, oldest, az, za'], 412);
            }
            $sort = $request->sort;
        }
    ...
    {
        if ($request->id == null) {
            return response()->json(['error' => 'ID is required'], 412);
        }
    ...
    public function saveDoc(Request $request)
    {
        ...
        if ($request->output == null) {
            return response()->json(['error' => 'Html Output is required'], 412);
        }
        if ($request->title == null) {
            return response()->json(['error' => 'Title is required'], 412);
        }
