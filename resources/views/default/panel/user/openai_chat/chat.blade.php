...
@section('content')
    ...
    <template id="chat_ai_bubble">
        <div class="lqd-chat-ai-bubble group mb-2 flex content-start gap-2">
            <span
                class="size-6 inline-block shrink-0 rounded-full bg-cover bg-center"
                style="background-image: url('{{ custom_theme_url($chat->category->image ?? custom_theme_url('assets/img/auth/default-avatar.png'), true) }}')"
            ></span>
            <div
                ...
                <div class="inline-flex items-center rounded-full font-medium leading-none">
                    ...
                    @endif
                    <pre class="chat-content relative w-full whitespace-pre-wrap px-6 py-3 indent-0 font-[inherit] text-xs font-normal [word-break:break-word] empty:hidden"></pre>
                    <button
