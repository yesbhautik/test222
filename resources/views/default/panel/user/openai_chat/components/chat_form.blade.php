...
<div
    class="lqd-chat-form-wrap sticky bottom-0 z-30 rounded-b-[inherit]"
    x-data="{
...
>
    ...
    >
                ...
                <div class="pointer-events-none absolute bottom-0 end-2 start-2 flex items-end justify-between py-[5px] text-sm max-md:static">
                    ...
                    >
                        <div @class([
                            ...
                            <button
                                class="lqd-chat-attach max-md:!text-heading size-10 flex shrink-0 cursor-pointer items-center justify-center gap-2 rounded-full bg-secondary text-secondary-foreground transition-all max-md:h-auto max-md:w-auto max-md:bg-transparent md:hover:bg-secondary md:hover:opacity-80"
                                type="button"
                                @if ($app_is_demo) onclick="return toastr.info({{ __('This feature is disabled in Demo version.') }})" @else id="chat_add_image" @endif
                            >
                                <x-tabler-paperclip
                                    class="size-5"
                                    stroke-width="1.5"
                        ...
                        <div class="pointer-events-auto flex items-center max-md:flex-col max-md:items-start max-md:gap-4 max-md:pb-4 md:ms-auto">
                            <button
                                class="size-10 flex shrink-0 cursor-pointer items-center justify-center gap-2 rounded-full text-heading-foreground transition-all max-md:h-auto max-md:w-auto max-md:bg-transparent md:hover:bg-secondary md:hover:text-secondary-foreground"
                                type="button"
                    ...
                    <div class="pointer-events-auto max-md:absolute max-md:bottom-[10px] max-md:end-2">
                        ...
                        <button
                            class="size-10 hidden shrink-0 cursor-pointer items-center justify-center gap-2 rounded-full text-heading-foreground transition-all max-md:h-auto max-md:w-auto max-md:bg-transparent md:hover:bg-secondary md:hover:text-secondary-foreground [&.active]:flex"
        ...
        />
        <x-button
            class="h-[52px] w-[52px] shrink-0 max-md:h-10 max-md:w-10"
            id="{{ $disable_actions ? '' : 'send_message_button' }}"
            size="none"
            tag="button"
            onclick="{!! $disable_actions ? 'return toastr.info(\'{{ __('This feature is disabled in Demo version.') }}\')' : '' !!}"
            type="submit"
        >
            <x-tabler-send-2
        ...
        <x-button
            class="hidden h-[52px] w-[52px] shrink-0 max-md:h-10 max-md:w-10 [&.active]:flex"
            id="stop_button"
            size="none"
            tag="button"
        >
