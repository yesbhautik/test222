...
<head>
    ...
    <meta
        name="stream-url"
        content="{{ $streamUrl ?? route('dashboard.user.openai.stream') }}"
    >
    <link
        rel="icon"
    ...
    @foreach (Theme::getSetting('googleFonts') as $font_name => $weights)
        @php
            $font_string = 'family=' . str_replace(' ', '+', $font_name);
            if (!empty($weights)) {
                $font_string .= ':wght@' . implode(';', $weights);
            }
            $google_fonts_string .= $font_string . ($loop->last ? '' : '&');
        @endphp
    @endforeach
