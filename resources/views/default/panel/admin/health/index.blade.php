                ...
                @if (count($checkResults?->storedCheckResults ?? []))
                    <dl class="mt-4 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach ($checkResults->storedCheckResults as $result)
                            <div
                                class="dark:bg-white-500 flex transform items-start space-x-2 overflow-hidden rounded-xl bg-[#fff] p-6 text-opacity-0 shadow-[0_7px_25px_rgba(0,0,0,0.07)] rtl:space-x-reverse dark:bg-opacity-5">
                                <div class="{{ backgroundColor($result->status) }} flex items-center justify-center rounded-full bg-opacity-25 p-2.5">
                                    <svg
                                        class="size-3 {{ iconColor($result->status) }}"
                                ...
                                <div>
                                    <dd class="-mt-1 font-semibold text-gray-800 dark:text-gray-200 md:text-xl">
                                        {{ $result->label }}
                                    </dd>
